...
let VerificationResolver = class VerificationResolver {
    verify(token) {
        return __awaiter(this, void 0, void 0, function* () {
            let decoded = (0, jsonwebtoken_1.verify)(token, `${process.env.JWT}`);
            if (typeof decoded != "string") {
                let user;
                try {
                    user = yield User_1.User.findOne({ where: { email: decoded.email } });
                    user.confirmed = true;
                    yield user.save();
                }
                catch (e) {
...
], VerificationResolver.prototype, "verify", null);
